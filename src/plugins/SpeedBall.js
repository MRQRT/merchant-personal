...
(function(global, factory) {
	...
	//实际的逻辑代码-确保只会被执行一遍
	//可以解决反复被引用问题
	if(!isEmptyObject(global.SpeedBall)) {
		//如果已经不是空对象，就不再次运行
		return global.SpeedBall;
	}
...
//			}, 3000);
		...
		function genrateColorByPercent(percent, type) {
			...
			if(type === 'inner') {
				//内圈的颜色
				rArray = [254, 254, 254, 254, 254, 254];
				//定义g颜色的几个值
				gArray = [156, 156, 156, 156, 156, 156];
				//定义b颜色的几个值
				bArray = [13, 13, 13, 13, 13, 13];
			} 
			else {
				//外圈的颜色
				//定义r颜色的几个值
				rArray = [0, 255, 160, 88, 88, 30];
				//定义g颜色的几个值
		...
		Effect.prototype.draw = function() {
			...
			ctx.beginPath();
			//默认用宽度的线
			ctx.lineWidth = options.lineWInner || 3;
			ctx.strokeStyle = '#ea9e21';
			...
			if($this.isShowPercent) {
				//字体
				ctx.font = options.percentFont || "20px italic";
				ctx.beginPath();
				if(options.isTxtHollow) {
					...
					ctx.fillText((percent * 100).toFixed(1) + "%", canvas.width / 2 - 14, canvas.height / 1.7);
				}
				ctx.closePath();
		...
		Effect.prototype.change = function(percent) {
			...
			var color2='#000' ;//= genrateColorByPercent($this.percent, 'txt');
			...
			$this.isShowPercent = options.isShowPercent;
